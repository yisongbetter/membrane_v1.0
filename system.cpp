...
void __stdcall ImageCallBack(unsigned char * pData, MV_FRAME_OUT_INFO* pFrameInfo, void* pUser)
{
	...
	//1.run algo
al_param *palgo_p=(al_param *)pUser;

	if(0==palgo_p->flag)
	{	
	//	nameWindow(palgo_p->res.my_label,CV_WINDOW_NORMAL);
		//imshow(palgo_p->res.my_label,palgo_p->res.defects);
		imshow(palgo_p->res.my_label,palgo_p->res.defects);

	}
	if(0==palgo_p->flag||-1==palgo_p->flag)
	{
		palgo_p->src=Image;
		palgo_p->flag=1;
	}
...
waitKey(1);
//destroyWindow(palgo_p->res.my_label)
...
////2222222222222222222222222222222222222222222222222
    ...
    nRet = MV_CC_RegisterImageCallBack(m_handle, ImageCallBack, (void*) &algo_p);
    if (MV_OK != nRet)
    {
...
///333333333333333333333333333333333333333
	...
	{	
		...
		if(1==algo_p.flag)
		{
			...
			int nRet=defects_detector(algo_p.src,&results);

			if(1==nRet)
			{
				
				algo_p.res.defects=results.defects;
				sprintf(algo_p.res.my_label,"%s",results.my_label);
